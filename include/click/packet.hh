...
class Packet { public:
    ...
    inline void set_psp_header(const click_psp *psph, uint32_t len);
  ...
  public:
	...
	dst_psp_anno_offset = 0, dst_psp_anno_size = 8,
	...
	inline uint64_t dst_psp_anno() const;
    inline void set_dst_psp_anno(uint64_t addr);
...
inline uint64_t
Packet::dst_psp_anno() const
{
    return xanno()->u64[dst_psp_anno_offset / 4];
}

inline void
Packet::set_dst_psp_anno(uint64_t a)
{
    xanno()->u64[dst_psp_anno_offset / 4] = a;
}
...
inline void
Packet::set_psp_header(const click_psp *psph, uint32_t len)
{
    set_network_header(reinterpret_cast<const unsigned char *>(psph), len);
}
